    ...
    switch (command) {
        case 'color':
            if(words[1].match('#[a-fA-F0-9]{6}')) {
                ...
                socket.emit('changeCookies', { key: 'color', value: newColor })
            ...
            break;
        case 'name':
            var cleanWords = text.replace(/\s+/g, ' ').trim().split(' ');
            cleanWords.shift();
            var newName = cleanWords.join(' ');
            if(newName.length == 0) {
                socket.emit('system message', { text: 'Empty name, read /help' });
            } else {
                socket.request.cookies['name'] = newName;
                socket.emit('changeCookies', { key: 'name', value: newName })
                socket.emit('setName', { name: newName });
                socket.emit('system message', { text: 'Name updated, read /help' });
            }
            break;
        case 'help':
            sendHelpMessage(socket);
...
function sendHelpMessage(socket) {
    socket.emit('system message', { text: 'Available commands: "/name new_name" - change name, "/color #012345" - change name color, "/help" - show this help' });
}